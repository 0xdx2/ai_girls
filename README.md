...
**macOS AI 桌面助手原型 —— Live2D 虚拟形象 × 本地 CLI 模型编排 × 可持续扩展的桌面交互**
...
AI Girls Desktop 是一个运行在 macOS 上的桌面助手项目，目标是把 **Live2D 虚拟角色表现**、**本地可调用的多模型 AI**、**桌面级工具能力** 和 **长期记忆** 融合到一个统一体验里。它不是单纯的聊天壳子，也不只是一个会动的看板娘，而是希望逐步演化成一个接近 **VS Code 中 GitHub Copilot Chat 那种全面能力** 的桌面 AI 工作台。

不过需要先说清楚：**当前仓库还处于“原型持续搭建阶段”**。现有版本已经完成了基础界面、Tauri + Rust 后端骨架、部分 CLI 模型探测与调用链路、基础工具运行时，以及 Live2D 的初步接入；但很多 README 里容易让人联想到“已经成熟可用”的能力，其实还只是部分完成，或者仍属于明确规划中的 TODO。这个版本更适合把它看成一个方向明确、架构已起步、功能仍在快速补齐的项目。
...
- 已实现本地 CLI 模型的优先级探测与 fallback 思路，支持从已安装命令中选择可用提供方。
...
项目当前采用的是 **本地 CLI 优先探测** 的思路：如果系统里安装了对应命令，就按优先级尝试调用；如果没有，再回退到其它可用项，最后才使用内置 mock 作为兜底。这个策略的好处是接入快、部署轻、便于在本地验证不同模型路径，但也意味着各家能力是否“真正可用”，不仅取决于项目代码，还取决于本地 CLI 本身的成熟度与调用方式是否稳定。

| 优先级 | 模型 | 调用方式 | 当前说明 |
|:---:|---|---|---|
| 1 | **Claude** (Anthropic) | `claude -p` | 已有适配思路，但整体体验仍待完善 |
| 2 | **Codex** (OpenAI) | `codex --approvals=never -q` | 已有入口，功能深度与稳定性仍需继续打磨 |
| 3 | **GitHub Copilot** | `gh copilot explain` | 当前相对最完整、最接近可持续使用的一条链路 |
| 4 | **Gemini** (Google) | `gemini -p` | 基础支持方向已留出，但能力仍偏早期 |
| 5 | Mock（离线备用） | 内置 | 用于无外部可用 CLI 时的兜底演示 |

这里特别强调一下：**这张表表达的是“适配优先级与现有接入方向”，不是“每个模型都已经做到了完整支持”**。如果你是为了寻找“本地一装即拥有全套多模型桌面 Copilot”而来，那目前这个仓库还没有到那个阶段；如果你想找一个把多模型桌面助手、角色交互和工具链整合到一起的工程原型，这个仓库正朝着那个目标在走。
...
项目当前优先走本地 CLI 调用，因此你安装了哪些命令，直接影响你实际能体验到哪些模型能力。需要再次提醒的是：安装命令并不自动等于“所有能力都已经成熟”，它只是让对应路径具备被探测和调用的机会。
...
│       ├── ai_adapters.rs        # 多模型 CLI 适配器与 fallback 链
